...
func resourcePostgreSQLGrantCreate(d *schema.ResourceData, meta interface{}) error {
	...
	objects, err := getAllObjectsType(txn, d)
	if err != nil {
		return err
	}

	if err = revokeRolePrivileges(txn, d, objects); err != nil {
		return err
	}

	if err = grantRolePrivileges(txn, d, objects); err != nil {
		return err
	}
...
// getAllObjectsType returns the list of all objects for the specified object_type
...
func getAllObjectsType(txn *sql.Tx, d *schema.ResourceData) ([]string, error) {
	...
	objectType := objectTypes[d.Get("object_type").(string)]
	rows, err := txn.Query(query, d.Get("schema").(string), objectType)
	if err != nil {
		return nil, errwrap.Wrapf("could get object list: {{err}}", err)
	}
	...
	for rows.Next() {
		...
		if err = rows.Scan(&name); err != nil {
			return nil, errwrap.Wrapf("could get scan row: {{err}}", err)
		}
...
func grantRolePrivileges(txn *sql.Tx, d *schema.ResourceData, objects []string) error {
	privileges := []string{}
...
func revokeRolePrivileges(txn *sql.Tx, d *schema.ResourceData, objects []string) error {
	query := fmt.Sprintf(
