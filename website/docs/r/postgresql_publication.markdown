...
- `schemas` - (Optional) Which schemas' tables should be added to the publication. Requires PostgreSQL 15 or above and superuser privileges. Can be combined with `tables`, but explicit tables must not belong to any listed schema.
